...
addGameForm.addEventListener("submit", e => {
  e.preventDefault();
  games.unshift({ // Changed from push to unshift
    id: Date.now(),
...
function renderFilteredGames(filteredGames) {
  ...
  <div class="game-card-content">
    ...
    <span class="status ${game.status}">${
      game.status === "done" ? "Пройдена" : 
      game.status === "want" ? "Хочу пройти" : "Отложена"
    }</span>
    <div class="stars" data-rating="${game.rating || 0}"></div>
    <textarea class="description" placeholder="Добавьте описание...">${game.description || ""}</textarea>
...
`;
      ...
      statusEl.addEventListener("click", () => {
        if (game.status === "done") game.status = "want";
        else if (game.status === "want") game.status = "postponed";
        else game.status = "done";
        saveData();
